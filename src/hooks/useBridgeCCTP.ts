  ...
  const bridge = useCallback(
    async ({ fromNetwork, toNetwork, amount }: BridgeParams) => {
      ...
      try {
        ...
        // - maxFee increased to 1000 to avoid "insufficient_fee" error (220 was not enough)
        ...
        const param1 = pad(toHex(1000), { size: 32 }); // maxFee for CCTP attestation (increased to 1000 to avoid insufficient_fee)
