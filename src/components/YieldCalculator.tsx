...
// Lock period info with boost multipliers (APY calculated dynamically)
const lockPeriodInfo = {
  flexible: { name: "Flexible", boost: 0, boostMultiplier: 1.0, duration: "Withdraw anytime" },
  "1month": { name: "1 Month Lock", boost: 17, boostMultiplier: 1.17, duration: "30 days" },
  "3months": { name: "3 Months Lock", boost: 35, boostMultiplier: 1.35, duration: "90 days" },
  "12months": { name: "12 Months Lock", boost: 69, boostMultiplier: 1.69, duration: "365 days" },
};
...
export const YieldCalculator: React.FC<YieldCalculatorProps> = ({
  ...
  const effectiveAPY = useMemo(() => {
    // Calculate APY based on base rate × boost multiplier
    return baseAPY * lockPeriodInfo[selectedPeriod].boostMultiplier;
  }, [selectedPeriod, baseAPY]);
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[10px] opacity-80">
                    {(baseAPY * info.boostMultiplier).toFixed(2)}% APY
                  </span>
                </Button>
              );
            })}
            ...
            <p className="text-xs text-muted-foreground">
              {selectedPeriod === "flexible" ? (
                ...
                <>
                  ...
                  Locked for {lockPeriodInfo[selectedPeriod].duration} (+
                  {lockPeriodInfo[selectedPeriod].boost}% boost)
                </>
              )}
            </p>
          </div>
