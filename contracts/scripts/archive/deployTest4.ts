...
async function main() {
  ...
  console.log("Deploying TestBridge4 (no return value)...");
  const artifact = await hre.artifacts.readArtifact("TestBridge4");
  ...
  const bridgeAbi = parseAbi(["function bridge(uint256 amount, bytes32 recipient) external"]);
  ...
  // Decode events
  const eventAbi = parseAbi(["event Done(string message)"]);
  for (const log of bridgeReceipt.logs) {
    if (log.address.toLowerCase() === testContract.toLowerCase()) {
      try {
        ...
        console.log("Event:", (decoded.args as any).message);
      } catch {}
    }
  }
