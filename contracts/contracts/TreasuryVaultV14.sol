...
/**
 * @title TreasuryVaultV14
 * @notice Treasury vault with gas optimization for activeDeposits
 * @dev V14 Changes:
 *   - Fixed _removeActiveDeposit: swap-and-pop to remove zero entries
 *   - Added cleanupActiveDeposits() for admin to clean bloated arrays
 *   - Prevents unbounded array growth that causes excessive gas costs
 */
contract TreasuryVaultV14 is
    Initializable,
    OwnableUpgradeable,
    ...
    UUPSUpgradeable
{
    ...
    function _removeActiveDeposit(address user, uint256 amount, bool isLocked) internal {
        ...
        // V14: Swap-and-pop to remove zero entries
        uint256 j = 0;
        while (j < deposits.length) {
            if (deposits[j].amount == 0) {
    ...
    /// @notice V14: Admin cleanup of bloated activeDeposits arrays (chunked)
    function cleanupActiveDeposits(address user, uint256 maxIterations) external onlyOwner {
        ActiveDeposit[] storage deposits = userActiveDeposits[user];
        uint256 iterations = 0;
        uint256 i = 0;
        while (i < deposits.length && iterations < maxIterations) {
            if (deposits[i].amount == 0) {
