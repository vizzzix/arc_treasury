...
/**
 * @title TreasuryVaultV11
 * @notice Treasury vault with auto USDC->USYC conversion
 * @dev V11 Changes:
 *   - Fixed mintUSYC to work with Arc native USDC precompile
 *   - Added approveUSDCForTeller() - vault can auto-convert deposits
 */
contract TreasuryVaultV11 is
    Initializable,
    OwnableUpgradeable,
    ...
    UUPSUpgradeable
{
    ...
    function withdraw(uint256 sharesToWithdraw) external whenNotPaused nonReentrant returns (uint256) {
        ...
        uint256 usdcToWithdraw = (totalUSDC * sharesToWithdraw * PRECISION) / totalShares / PRECISION;
        require(usdcToWithdraw > 0 && usdcToWithdraw <= totalUSDC, "Invalid withdraw");
