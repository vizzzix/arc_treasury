...
/**
 * @title TreasuryVaultV12
 * @notice Treasury vault with automatic USDC<->USYC conversion
 * @dev V12 Changes:
 *   - Auto-convert USDC to USYC on deposit (if Teller is set and autoConvert enabled)
 *   - Auto-redeem USYC to USDC on withdraw (if needed)
 *   - Withdraw calculates from total pool value (USDC + USYC)
 */
contract TreasuryVaultV12 is
    Initializable,
    OwnableUpgradeable,
